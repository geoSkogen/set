class Gameboard {
  ...
  createCardElement(card_obj) {
    if (card_obj.constructor.name==='Card') {
      ...
      card_el.id = card_obj.getDeckKey
  ...
  removeCardElement(card_obj) {
    if (this.#appElement.getElementById(card_obj.deck_id)) {
      this.#appElement.removeChild( this.#appElement.getElementById(deck_id))
    } else {
      console.log('Gameboard::removeCardElement expects the ID of a child element of "#app"')
    }
  }
